  ...
  try {
    ...
    for (const attrFilter of attributes) {
      ...
      let whereConditions: any = {
        attributeId: { id: attrFilter.attribute_id },
        entity: "property",
      };
      ...
      // استخراج معرفات العقارات من attribute_value
      const propertyIds = new Set(matchedValues.map((v) => v.entity_id));
      propertyIdsSets.push(propertyIds);
    }

    // دمج مجموعات العقارات مع شرط تقاطع (أي العقارات التي تحقق كل الشروط)
    ...
    if (propertyIdsSets.length > 0) {
      ...
      finalPropertyIds = new Set(); // لا فلترة على attributes
    }

    // بناء شرط where للعقارات
    ...
    if (finalPropertyIds.size > 0) {
      ...
      // إذا هناك attributes ولكن لا عقارات تطابقهم، نرجع نتيجة فاضية
      return res.json(ApiResponse.success([], "No properties found", { total: 0, page, limit, totalPages: 0 }));
    }

    // جلب العقارات مع Pagination
