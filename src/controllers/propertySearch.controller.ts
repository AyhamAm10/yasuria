  ...
  try {
    ...
    if (attributes && attributes.length > 0) {
      for (const [index, attr] of attributes.entries()) {
        ...
        const attributeEntity = await AppDataSource.getRepository(Attribute).findOne({
          where: { id: attr.attribute_id },
        });

        if (!attributeEntity) {
          throw new APIError(HttpStatusCode.BAD_REQUEST, `Attribute with id ${attr.attribute_id} not found`);
        }
        ...
        if (attributeEntity.input_type === 'dropdown') {
          ...
          if (attr.min !== undefined) {
            query.andWhere(`${alias}.value >= :attrMin${index}`, { [`attrMin${index}`]: `${attr.min}` });
          }
          if (attr.max !== undefined) {
            query.andWhere(`${alias}.value <= :attrMax${index}`, { [`attrMax${index}`]: `${attr.max}` });
          }
        }
      }
    }
